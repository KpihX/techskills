...
│   ├── regex_tutorial.md
│   └── …
└── AGENTS.md               ← this file (project rules; CLAUDE.md / GEMINI.md symlinks + .agent/AGENT.md currently absent from working tree — see TODO/proposal to restore)

> * `system/bw-env.md` + `apps/tg.md` = planned (see TODO.md) — sidebar/README link to future homes.
> Links are base-relative from repo root (`system/clean.md`, never `../`) — Docsify resolves from base.
```
...
- `system/` ← Ubuntu core (automount, clean, zsh_env, nautilus, ubuntu-*, win11_usb, wine, bw-env)
- `dev/` ← dev toolchain (gh, glab, github-pages, npm-prefix, bun, fnm, azure-cli)
- `net/` ← network & cloud (tailscale, cloudflare, cloud-hub)
